...
export function InheritanceSimulator({ rows }: { rows: PersonWithScore[] }) {
  ...
  return (
    <section className="relative overflow-hidden rounded-app border border-gold/25 bg-gradient-to-br from-gold/15 via-white/[0.06] to-ink p-5">
      ...
      <div className="relative">
        ...
        <h2 className="mt-1 font-serif text-3xl font-black text-ivory">Projected Inheritance Simulator</h2>
        <div className="mt-4 grid gap-3">
          {!visibleRows.length ? (
            <p className="text-sm font-semibold text-champagne/70">
          ...
          {visibleRows.map((row) => {
            ...
            return (
              <div key={row.id}>
                <div className="flex justify-between gap-4 text-sm font-bold text-ivory">
                  <span>{row.name}</span>
        ...
        <p className="mt-4 text-xs font-semibold text-champagne/60">
