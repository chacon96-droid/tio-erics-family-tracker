  ...
  return (
    <AppShell>
      <div className="grid gap-6">
        <section>
          <p className="text-xs font-black uppercase text-clay">Overview</p>
          <h2 className="text-3xl font-black">Dashboard</h2>
        </section>
        <section className="grid gap-3 md:grid-cols-4">
          <StatCard label="Active people" value={people.filter((person) => person.active).length} />
          <StatCard label="Approved this month" value={approved.length} />
          <StatCard label="Pending approvals" value={pending.length} />
          <StatCard label="Top score" value={leaderboard[0]?.score?.total_score || 0} detail={leaderboard[0]?.name || "No leader yet"} />
        </section>
        <section>
          <div className="mb-3 flex items-end justify-between gap-4">
            <div>
              <p className="text-xs font-black uppercase text-clay">This month</p>
